...
### The number of nodes in the tree, and the maximum value for the random number that generates the node current value
NUM_OF_NODES = 100
MAX_NUM = 100
...
class Node(object):
    def __init__(self, parent=None, current=None, right=None, left=None, depth=None):
        self.type = None
        self.parent = parent
    ...
    def __str__(self):
        return "NODE - current: %d depth: %d" % (self.current, self.depth)
    ...
    def draw(self):
        ...
        if BOX_SIZE[0] >= 10: # skip text if box is too small
            text = FONT.render(str(self.current), 1, WHITE)
            tr = text.get_rect()
    ...
    for event in pygame.event.get():
        if event.type == QUIT:
            ...
            if event.key in [K_q, K_ESCAPE]:
                ...
                insert_node(leaf.left, current, depth+1)
            else:
...
def draw():
    ...
    if m.selection:
        ...
        # Node Info
        text = FONT2.render("Node current: %d, depth: %d" % (m.selection.current, m.selection.depth), 1, (200, 255, 255))
        trect = text.get_rect()
...
def create_root_node(current):
    root = Node(current=current, depth=0)
    root.type = "root"
    add_new_node(root, 0)
    m.nodecount += 1
    return root
...
def insert_node(leaf, current, depth = 0):
    if not leaf.current:
        leaf.current = current
        leaf.depth = depth
    elif current < leaf.current:
        if not leaf.left:
        ...
        insert_node(leaf.right, current, depth + 1)
    elif current == leaf.current:
